...
serve(async (req) => {
  ...
  try {
    ...
    // 🔥 FIX: Handle both "/" and "/login-api/" as root
    const isRoot = path === '/' || path === '/login-api/' || path === '/login-api'
    ...
    return new Response(JSON.stringify({
      ...
      path: path,
      method: req.method,
      message: 'Check the path and method. Root paths: /, /login-api/, /login-api',
      available_endpoints: [
        'GET / or /login-api/ - Test endpoint',
        'POST /api/submit - Submit login data',
        'GET /api/requests - Get all requests (admin)',
        'POST /api/approve - Approve/deny request (admin)',
